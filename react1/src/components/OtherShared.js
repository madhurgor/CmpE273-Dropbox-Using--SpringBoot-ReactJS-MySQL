...
class OtherShared extends Component {
  ...
  onDownload = (item) => {
    axios.get(`http://localhost:8080/users/downloadG`,{params:{file:item,username:this.props.select.group_s.creator,folder:this.props.select.group_s.folders}})
       .then((res) => {
         //console.log(res);
  ...
  onFilesUpload = () => {
    if(this.state.files.length>0){
      ...
      axios.post(`http://localhost:8080/users/filesO`, formData, {params:{'username':`${this.props.select.group_s.creator}`,folder:this.props.select.group_s.folders}})
      .then(response => {
