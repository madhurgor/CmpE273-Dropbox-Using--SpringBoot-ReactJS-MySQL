...
class OwnShared extends Component {
  ...
  onShareFile = () => {
      axios.get(`http://localhost:8080/users/shareO`,{params:{file:fileShare,username:this.props.select.username,folder:this.props.select.group_s.folders,member:[document.getElementById('s1').value,document.getElementById('s2').value,document.getElementById('s3').value,document.getElementById('s4').value,document.getElementById('s5').value,]}})
         .then((res) => {
           //console.log(res);
           console.log('shared..');
  ...
  onDownload = (item) => {
      axios.get(`http://localhost:8080/users/downloadG`,{params:{file:item,username:this.props.select.username,folder:this.props.select.group_o}})
         .then((res) => {
           //console.log(res);
  ...
  onDelete = (item) => {
    if(window.confirm('Delete File?')){
      axios.get(`http://localhost:8080/users/deleteG`,{params:{file:item,username:this.props.select.username,folder:this.props.select.group_o}})
         .then((res) => {
           console.log('deleted..');
  ...
  onFilesUpload = () => {
    if(this.state.files.length>0){
      ...
      axios.post(`http://localhost:8080/users/filesO`, formData, {params:{'username':`${this.props.select.username}`,folder:this.props.select.group_o}})
      .then(response => {
