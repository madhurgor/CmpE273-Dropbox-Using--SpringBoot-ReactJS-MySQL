...
class Groups extends Component {
  ...
  onOwnDelete = (item) => {
    if(window.confirm('Delete Shared Folder?')){
      axios.get(`http://localhost:8080/users/delete_own`,{params:{ownfolder:item,username:this.props.select.username,}})
         .then((res) => {
           console.log('deleted..');
  ...
  onFilesUpload = () => {
    if(this.state.files.length>0){
      ...
      axios.post(`http://localhost:8080/users/filesF`, formData, {params:{'username':`${this.props.select.username}`,}})
      .then(response => {
